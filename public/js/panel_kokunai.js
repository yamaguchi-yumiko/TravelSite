(function (win, $) {
	...
	// バージョンチェック
	args.form = args.form || 'form_search_dom,form_search_kokunai,form_search_kokunaidp';
	...
	$.fn.searchPanelTab = function (option) {
		...
		return this.each(function () {
			...
			var $tabFocusOpen = $tabPanel.children('.js_focus_open').find('input');
			var $tabOpenBtn = $root.find('.js_panel_open');
			var $tabCloseBtn = $root.find('.js_panel_close');
			...
			// タブ操作
			$tabOpenBtn.on('click.slideOpen', function () {
				$(this).closest('.panel_item').removeClass('-is_close').addClass('-is_open');
			});

			// タブ内条件追加操作
			$tabCloseBtn.on('click', function () {
				$(this).closest('.panel_item').removeClass('-is_open').removeClass('-is_active');
			});

			// 国内ダイナミックのみfocus後開く
			$tabFocusOpen.on('focus', function () {
				$(this).closest('.panel_item').removeClass('-is_close').addClass('-is_open');
			});

			$tabPanelItem.on('transitionend', function () {
				var isOpen = $(this).hasClass('-is_open');
				if (!isOpen) {
					$(this).closest('.panel_item').addClass('-is_close');
				} else {
					$(this).closest('.panel_item').addClass('-is_active');
				}
			});
		});
	};
										...
										if ($this[0].selectedIndex !== undefined) {
											if (dat.push) {
												...
												if ($this.selectmenu && $this.selectmenu('instance')) {
													...
													lbl && console.log([this, lbl]) && $this.val(lbl);
												}
											}
										} else if ($this.hasClass('js_suggest')) {
											// ボタンパネルの有効無効制御
											var $drp = $this.parent().find(".js_drop_item");
											$drp.find(".js_suggest_btn").each(function () {
												var $this = $(this);
											...
											$drp.find(".tab_search .tab_list a").each(function () {
												var $this = $(this);
	...
	$.fn.suggest = function (_option) {
		return this.each(function () {
			...
			var $dropItemLastFocusable = $dropItem.find("a[href], button:not(:disabled)").last();
			var $body = $("body");
			var val = $root.val();
			...
			$root.on('focus', function (e) {
				...
				$body.on('click.closeSuggest', function (e) {
					var $target = $(e.target);
					if (!$root.is($target) && !$target.closest(".ui-widget").length) {
						$root.trigger('closeSuggest');
					} else {
						$frms.find(".js_drop_item").not($dropItem).removeClass('-is_open');
					}
				});
			});
			...
			// dropdown内最後の要素から外れた場合
			$dropItemLastFocusable.on('keydown', function (e) {
			...
			$root[0].refresh = function (_data) {
				// var dat = _data;
				// if (dat && false) { // まだ動かさない
				// 	var $this = $(this);
				// 	$this.each(function () {
				// 		if (this.getAttribute('data-value') in dat) {
			...
			$dropItem.on('click', '.js_suggest_btn', function () {
				...
				//console.log([$root[0], this, $this.attr('data-value-keyword'), $this.attr('data-label-keyword')]);
				val && $root
					// 値設定
					.attr('data-value', val)
					// 表示名項目を反映する
		...
		return this.each(function () {
			...
			$input.on('focus', function (e) {
				...
				$body.on('click.closeDropSelect', function (e) {
					var $target = $(e.target);
					if (!$input.is($target) && !$target.closest(".ui-widget").length) {
	 ...
	 * multiselect
	 */
	$.fn.setMultiselect = function (_option) {
		return this.each(function () {
			var $this = $(this);
			var opt = {
				header: false,
				height: 300,
				selectedList: $this.prop('length'),
				noneSelectedText: $this.attr('data-placeholder'),
				position: {
					my: 'right top-1', at: 'right bottom'
				},
				click: function (_$event, _ui) {
					var $this = $(this);
					var tgt = _$event.toElement;
					if (tgt.value) {
						$this.multiselect('widget').find(":checkbox[value='']:checked").click();
					} else {
						$this.multiselect('widget').find(":checkbox[value!='']:checked").click();
					}
				}
			};
			// data-source 指定がある場合は AJAX で読み込む
				...
				$.ajax({
					...
					success: function (_data) {
						...
						if (arr && arr.push) {
							...
							for (var idx = 0, max = arr.length; idx < max; idx++) {
								var itm = arr[idx];
								if (itm.group && grp !== itm.group) {
									grp = itm.group;
									$grp = $('<optgroup label="' + grp + '">');
									$this.append($grp);
								}
								$grp.append('<option value="' + itm.name + '">' + itm.label);
							}
							$this.find(".js_multiple_select").multiselect(opt);
						}
					}
				});
			} else {
	...
	$(function () {
		$frms.on('focus', 'input[type="text"], select + span', function () {
			$(this).closest('.form_item').addClass('-is_active');
			$(this).closest('.form_item_select').addClass('-is_active');
		}).on('focusout', 'input[type="text"], select + span', function () {
			$(this).closest('.form_item').removeClass('-is_active');
			$(this).closest('.form_item_select').removeClass('-is_active');
		...
		$frms.find(".js_select").each(function () {
			...
			$this.selectmenu({
				...
				select: function (_event, _ui) {
					...
					var org = this.getAttribute('data-value') || '';
					if (org !== val) {
						trg = true;
						this.setAttribute('data-value', val);
					}
					var org = this.getAttribute('data-label') || '';
					var val = opt && (opt.textContent || opt.innerText) || '';
					if (org !== val) {
					...
					if (trg) {
						...
						if (hoks && hoks.onChange) {
							hoks.onChange($(frm), $this);
						}
					}
				}
			});
		});
		$frms.find(".js_multiple_select").setMultiselect();
		...
		(function calendar() {
			...
			var defaults = {
				...
				beforeShow: function (input, inst) {
					...
					setTimeout(function () {
						$calendar.css({ top: rect + height + 1 });
					}, 1);
				},
			...
			$frms.find(".js_datepicker").each(function () {
				...
				var $syncTo;
				var linkage = $this.attr('data-datepicker-linkage-from');
				var $linkageTo;
				// 宿泊日 + 泊数
				var settinglinkage = {
					onSelect: function (_text, _instance) {
						$.datepicker.onSelectSuper.call(this, _text, _instance);
						var selectIndex = $linkageTo.prop('selectedIndex');
						if (selectIndex === 0) {
							$linkageTo.find('option').eq(0).prop('selected', 'selected');
							$linkageTo.selectmenu('refresh');
						}
					}
				};
				// 出発日
				...
				var settingSyncTo = {
					...
					onClose: function (selectedDate, inst) {
						//$this.datepicker('option', 'maxDate', selectedDate? selectedDate : '+1y');
					}
				};
				...
				if (sync) {
					...
					}, settingSyncFrom)).attr('data-datepicker-initialized', true);
					$syncTo.datepicker($.extend(true, {}, defaults, {
						minDate: $syncTo.attr('data-min') || defaults.minDate,
						maxDate: $syncTo.attr('data-max') || defaults.maxDate
					}, settingSyncTo)).attr('data-datepicker-initialized', true);
				} else if (linkage) {
					$linkageTo = $frms.find("[data-datepicker-linkage-to='" + linkage + "']");
					$this.datepicker($.extend(true, {}, defaults, {
						minDate: $this.attr('data-min') || defaults.minDate,
						maxDate: $this.attr('data-max') || defaults.maxDate
					}, settinglinkage)).attr('data-datepicker-initialized', true);
				} else {
					$this.datepicker($.extend(true, {}, defaults, {
						minDate: $this.attr('data-min') || defaults.minDate,
						maxDate: $this.attr('data-max') || defaults.maxDate
		...
		(function autocomplete() {
			$frms.find(".js_autocomplete").each(function () {
				...
				// サジェストから選択した場合Enterで検索させる
				$(document).on('keypress', function (e) {
					if ($this.hasClass('-is_selected') && e.which == 13) {
						$('#form_search_dom .btn.-search').trigger('click');
					}
				});
				...
				if (!$this.attr('data-ajax-autocomplete')) {
					$this.autocomplete({
						...
						source: function (_request, _response) {
							$.ajax({
								url: '/kokunai_dp/spookserver?Command=StayplaceSuggest&suggestkeyword=' + encodeURI(_request.term) + '&suggesttype=stayplace&showrrbchiku=1&showtycode=1',
								dataType: 'json',
								type: 'GET',
								success: function (_data) {
									var ret = [];
									...
									if (arr) {
										for (var idx = 0, max = arr.length; idx < max; idx++) {
											...
											ret.push({
												hotel: itm.value,
												area: itm.suggest_code,
												location: itm.pref_code,
												prefname: itm.display_pref_name,
												type: {
													area_master: 'pin',
													area_child: 'child',
													landmark_master: 'flag',
													landmark_station: itm.suggest_name.slice(-2) == '空港' ? 'plane' : 'train',
													airport: 'plane',
													custom_control_shisetsu: 'hotel',
													stayplace_master: 'hotel',
													stayplace_dictionary_tycode: 'star',
													stayplace_dictionary_area: 'pin'
												}[itm.data_type] || 'pin',
												label: itm.suggest_name,
						...
						select: function (_event, _ui) {
							...
							var ico = _ui.item.type;
							var typ = _ui.item.data_type;
							$this.attr({
								'data-label': lbl,
						...
						if ($this.val() !== $this.attr('data-label')) {
							// 何か文字を入力した場合は確定内容を破棄
							$this.removeAttr('data-label data-value data-landmark data-datatype').removeClass('-is_selected');
						}
						...
						if (!$this.attr('data-value')) {
							dbg && console.log($($this[0].form).find('#search_dom_place').val());
							dbg && console.log(window.checkitems);
						}
						...
						if (!itm.prefname) {
							...
							itm.prefname = "<span>,</span>" + itm.prefname;
						}
						return $("<li class='ui-menu-item'><a tabindex=-1 class='ui-menu-item-wrapper'><span class='icon -" + (itm.type || 'pin') + "'><span class='txt_strong'>" + "<em class='txt_em'>" + itm.label + "</em>" + itm.prefname + "</span>").appendTo(_$ui);
					};
						...
						if (vals) {
							...
							switch (elm.type) {
								...
								case 'radio':
									...
									break;
								default:
									if (elm.getAttribute('data-datepicker-initialized')) {
										elm.setAttribute('data-value', val.replace(/\//g, ''));
										$(elm).datepicker('setDate', lbl);
									} else {
										elm.value = lbl;
										elm.setAttribute('data-value', val);
									}
				...
				if (reg.test(frm.className) && frm.action) {
					submit(frm, function (_event) {
						...
						if (ok) {
							...
							if (frm.acceptCharset && frm.acceptCharset !== 'UNKNOWN' && /\b(Trident|MSIE|Edge)\b/.test(navigator.userAgent)) {
								// IE/Edge で文字コードが SJIS の商材(海外ツアー)対策
								setTimeout(function () {
	...
	(function calendaredit() {
		...
		// 監視対象の要素オブジェクト
		var elem = document.querySelector('#search_dom_checkin_day');
		var target = document.querySelector('#search_dom_stay');

		// 自動で1泊が入る
		function autostaydate() {
			if (!Element.prototype.matches) {
				Element.prototype.matches = Element.prototype.msMatchesSelector;
			}
			//デートピッカーがクリックされたとき
			var stayMenu = document.getElementById('search_dom_stay-button');
			document.getElementById('ui-datepicker-div').addEventListener('mouseup', function (e) {
				setTimeout(function () {
					// カレンダー用フラグが全てtrueになったら
					if (windowflag == true && calendarflag == true) {
						//クリックイベントが走ってかつ泊数が初期値の場合のみ処理
						if (document.querySelector('li._dom a.-is_active') && document.querySelector('span.ui-selectmenu-text').textContent == '泊数' && target && stayMenu) {
							//change 泊数 to one
							if (e.target.matches('#ui-datepicker-div td[data-handler="selectDay"] a') || e.target.matches('#ui-datepicker-div td[data-handler="selectDay"]')) {
								target.value = '1';
								stayMenu.click();
								stayMenu.click();
							}
		...
		// ウインドウの読み込みが終わったらフラグを切り替える
		$(window).on('load',function(){
			windowflag = true;
		});

		// ブラウザバック時にdisabledを入れる
			...
			$.widget('app.selectmenu', $.ui.selectmenu, {
				_drawButton: function () {
					...
					if (!selected && placeholder) {
						this.buttonItem.html('<span class="-is_empty">' + placeholder + '</span>');
					}
				}
			});
		})(jQuery);
