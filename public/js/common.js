...
(function () {
	(function onInitialize(_callback) {
		...
		if (doc.addEventListener) {
			...
			doc.addEventListener('reactRouteChangeComplete',_callback,false);
		...
		(function defineDegradedFunctions() {
			window.getExpDate_2012 = function (_days, _hours, _minutes) {
				console.warn('getExpDate_2012 is deprecated.');
			...
			window.getCookieVal_2012 = function (_offset) {
	            console.warn('getCookieVal_2012 is deprecated.');
			...
			window.getCookie_2012 = function (_name) {
				console.warn('getCookie_2012 is deprecated.');
				var arg = _name + '=';
				var alen = arg.length;
			...
			window.setCookie_2012 = function (_name, _value, _expires, _path, _domain, _secure) {
			    console.warn('setCookie_2012 is deprecated.');
			...
			window.deleteCookie_2012 = function (_name, _path, _domain) {
				console.warn('deleteCookie_2012 is deprecated.');
            ...
            // 居住地設定
            window.setPref = function (_pref, _type) {
                if (isFinite(_pref)) {
                    document.cookie = [
                        (_type || 'LivePrefCode') + '=' + _pref,
                        'path=/',
                        'domain=.jtb.co.jp',
                        'expires=' + (function () {
                            var ret = new Date();
                            ret.setFullYear(ret.getFullYear() + 1);
                            return ret.toGMTString();
                        })()
                    ].join('; ');
                }
            };
