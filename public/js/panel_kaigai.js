(function (win, $) {
	...
	// バージョンチェック
	args.form = args.form || 'form_search_kaigai,form_search_kaigaiair';
	if (args.version !== VERSION) {
		console.warn('Version unmatched ' + args.version + ' and ' + VERSION);
	} else {
		console.log('panel_kaigai.js version ' + VERSION);
	}
	// フォーム取得
				...
				if ($this.attr('data-ajax-autocomplete')) {
					$this.autocomplete({
						source: function (_request, _response) {
							$.ajax({
								url: this.element.attr('data-ajax-autocomplete') + '?' + encodeURIComponent(_request.term),
								dataType: 'json',
								type: 'GET',
								success: function (_data) {
									var ret = [];
