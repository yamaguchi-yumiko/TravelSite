...
<div class="reserve_container">
    ...
    <div class="reserve_content">
        ...
        <div class="hotel_info">
            <p>{{$roomInfoData['hotel_name']}}<span>({{$roomInfoData['hotel_name_kana']}})</span></p>
            <table>
                <tr>
                    <th>商品名</th>
                    <td>{{$roomInfoData['room_stick']}}</td>
                </tr>
                <tr>
                    <th>利用日</th>
                    <td>{{$date}}</td>
                ...
                <tr>
                    <th>泊数</th>
                    <td>{{$roomInfoData['stay_number']}}泊</td>
                </tr>
                <tr>
                    <th>人数・室数</th>
                    <td>{{$roomInfoData['guest_number']}}名{{$roomInfoData['rooms_number']}}室</td>
                </tr>
                <tr>
                    <th>プラン条件</th>
                    <td>{{$roomInfoData['stay_number']}}泊{{$roomInfoData['rooms_food']}}</td>
                </tr>
            </table>
        </div>
            ...
            <div class="travel_info">
                ...
                <div class="price_items">
                    ...
                    <table>
                        ...
                        @foreach($gest as $index => $ges)
                        <tr>
                            <th colspan="2" rowspan="2">部屋{{$ges['room']}}</th>
                        </tr>
                        <tr class="price_items_detail">
                            <td>{{number_format($ges['adults_price'])}}円</td>
                            <td>{{$ges['adults_number']}}</td>
                            <td>{{number_format($ges['child_price'])}}円</td>
                            <td>{{$ges['child_number']}}</td>
                            <td>{{number_format($ges['sub_price'])}}円</td>
                        </tr>
                        @endforeach
                        <tr>
                            <td colspan="7" class="price_items_subtotal">
                                宿泊料金合計 <span>{{number_format($total)}}円</span>
                            </td>
                        </tr>
                        <tr>
                ...
                <div class="subscriber_info">
                    ...
                    <table>
                        ...
                        <tr>
                            <td>氏名（漢字）</td>
                            <td>{{$sessionUser['last_name']}}&nbsp;{{$sessionUser['first_name']}}&nbsp;様</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>氏名（カタカナ）</td>
                            <td>{{$sessionUser['last_name_kana']}}&nbsp;{{$sessionUser['first_name_kana']}}&nbsp;様</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>性別</td>
                            <td>{{$sessionUser['gender'] === 1 ? '男性': '女性'}}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>年齢</td>
                            <td>{{floor((date('Ymd') - $sessionUser['birthday']) / 10000)}}歳</td>
                            <td></td>
                        </tr>
                        <tr class="address_info">
                            <td rowspan="2">住所<span>※必須</span></td>
                            <td class="postal_code">
                                郵便番号<input type="text" name="postal_code_first" @focus="postalFirstFocus" :class="!isPostalFirst ? postalFirstClass : 'on_check'" value="{{old('postal_code_first') ? old('postal_code_first') : substr($sessionUser['postal_code'],0,3)}}">
                                - <input type="text" name="postal_code_second" @focus="postalSecondFocus" :class="!isPostalSecond ? postalSecondClass : 'on_check'" value="{{old('postal_code_second') ? old('postal_code_second') : substr($sessionUser['postal_code'],4,4)}}"><br>
                                <p class="error">
                        ...
                        <tr class="address_info">
                            <td class="prefectures">
                                ...
                                <select name="address_prefectures" @focus="prefecturesFocus" ref="prefectures" value="{{ old('address_prefectures') }}" :class="!isPrefectures ? PrefecturesClass : 'on_check'">
                                    ...
                                    <option value="{{$index}}" @if(old('address_prefectures')==$index) selected @elseif($sessionUser['prefecture_cd']==$index) selected @endif>{{ $address }}</option>
                                    @endforeach
                                </select>
                                <p class="address_cities">市区町村<input type="text" name="address_cities" ref="cities" @focus="citiesFocus" :class="!isCities ? citiesClass : 'on_check'" value="{{ old('address_cities') ? old('address_cities') : $sessionUser['city_name'] }}"></p>
                                <p class="address_town">町名番地<input type="text" name="address_town" @focus="townFocus" :class="!isTown ? townClass : 'on_check'" value="{{ old('address_town') ? old('address_town') : $sessionUser['town_name']}}"></p>
                                <p class="address_other">マンション名など<input type="text" name="address_other" value="{{ old('address_other') ? old('address_other') : $sessionUser['other_address']}}"></p>
                                <p class="error">
                        ...
                        <tr class="telephone">
                            ...
                            <td class="telephone_item">
                                <input type="telephone" name="tel_first" @focus="telFirstFocus" :class="!isTelFirst ? telFirstClass : 'on_check'" value="{{old('tel_first') ? old('tel_first') : substr($sessionUser['tel'],0,3)}}">-
                                <input type="telephone" name="tel_second" @focus="telSecondFocus" :class="!isTelSecond ? telSecondClass : 'on_check'" value="{{old('tel_second') ? old('tel_second') : substr($sessionUser['tel'],5,3)}}">-
                                <input type="telephone" name="tel_third" @focus="telThirdFocus" :class="!isTelThird ? telThirdClass : 'on_check'" value="{{old('tel_third') ? old('tel_third') : substr($sessionUser['tel'],9,4)}}">
                                <p class="error">
                        ...
                        <tr class="telephone emergency_contact">
                            <td>電話番号<br>（緊急連絡先）</td>
                            <td class="telephone_item">
                                <input type="telephone" name="emergency_contact_first" @focus="emergencyFirstFocus" :class="!isEmergencyFirst ? emergencyFirstClass : 'on_check'" value="{{old('emergency_contact_first') ? old('emergency_contact_first') : substr($sessionUser['emergency_contact'],0,3)}}">-
                                <input type="telephone" name="emergency_contact_second" @focus="emergencySecondFocus" :class="!isEmergencySecond ? emergencySecondClass : 'on_check'" value="{{old('emergency_contact_second') ? old('emergency_contact_second') : substr($sessionUser['emergency_contact'],5,3)}}">-
                                <input type="telephone" name="emergency_contact_third" @focus="emergencyThirdFocus" :class="!isEmergencyThird ? emergencyThirdClass : 'on_check'" value="{{old('emergency_contact_third') ? old('emergency_contact_third') : substr($sessionUser['emergency_contact'],9,4)}}">
                                <p class="error">
                ...
                <div class="delegate_info">
                    ...
                    <table>
                        ...
                        <tr class="delegate">
                            ...
                            <td></td>
                            @else
                            <td>部屋{{ $gest['room'] }}&nbsp;代表者氏名（漢字）<br><span>※必須</span></td>
                            <td>
                                <div class="delegate_item">
                                    <p>姓<input type="text" name="first_name[{{$index}}]" value='{{old("first_name.$index")}}'></p>
                                    <p>名<input type="text" name="last_name[{{$index}}]" value='{{old("last_name.$index")}}'></p>
                                </div>
                                @if($errors->has("first_name.$index"))
                                <p class="error" ref="gestFirstError">部屋{{ $gest['room'] }}{{$errors->first("first_name.$index")}}</p>
                                @elseif($errors->has("last_name.$index"))
                                <p class="error" ref="gestlastError">部屋{{ $gest['room'] }}{{$errors->first("last_name.$index")}}</p>
                                @else
                                <p><span>部屋{{ $gest['room'] }}&nbsp;代表者氏名（漢字）は必ず入力してください。</p>
                                @endif
